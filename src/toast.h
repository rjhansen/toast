...
template <std::random_access_iterator Src,
    ...
    std::relation<std::iter_value_t<Src>, std::iter_value_t<Src>> Cmp = std::less<std::iter_value_t<Src>>>
    ...
    Cmp cmp = std::less<std::iter_value_t<Src>>())
{
    ...
    quicksort(greater_eq.begin(), std::back_inserter(sorted_ge), greater_eq.size(), cmp);
